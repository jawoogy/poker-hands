...
internal class PokerHand 
{
    ...
    {
        var groups = CardList.GroupBy(card => card.NumValue)
        ...
        switch (groups.Count)
        {
            ...
            case 3: // Two pair or three of a kind
                ...
                HandEval = Rank switch
                {
                    3 => $"Two pair, {DescribeFace(groups.Where(g => g.Count() == 2).OrderByDescending(g => g.Key).First().Key)}s and {DescribeFace(groups.Where(g => g.Count() == 2).OrderByDescending(g => g.Key).Last().Key)}s",
                    4 => $"Three of a kind, {DescribeFace(groups.OrderByDescending(g => g.Count()).First().Key)}s",
            ...
            case 2: // full house and four of a kind
                ...
                HandEval = Rank switch
                {
                    8 => $"Four of a kind, {DescribeFace(groups.OrderByDescending(g=>g.Count()).First().Key)}s",
                    7 => $"Full house, {DescribeFace(groups.OrderByDescending(g=>g.Count()).First().Key)}s over {DescribeFace(groups.OrderByDescending(g=>g.Count()).Last().Key)}s",
                    _ => throw new ArgumentException("Invalid hand: expected either full house or four of a kind, but found neither.")
                };
            break;

            case 1:
            throw new ArgumentException("Invalid hand: all cards are the same. Someone is cheating!");
    ...
    private int Check5CardHand()
    {
        ...
        var isStraight = CardList[0].NumValue - CardList[4].NumValue == 4 
    ...
    private int CheckFullHouseOrFourOfAKind(List<IGrouping<int, Card>> groups)
    {
        ...
        throw new ArgumentException("Invalid hand: expected either full house or four of a kind, but found neither.");
    }
    ...
    public (int result, string? highCard) CompareHand(PokerHand? other)
    {
        ...
        else
        {
            ...
            {
                var thisGroup = CardList.GroupBy(c => c.NumValue).OrderByDescending(g => g.Count()).First();
                var otherGroup = other.CardList.GroupBy(c => c.NumValue).OrderByDescending(g => g.Count()).First();
            ...
            {
                var thisGroups = CardList.GroupBy(c => c.NumValue).Where(g => g.Count() > 1).OrderByDescending(g => g.Key).ToList();
                var otherGroups = other.CardList.GroupBy(c => c.NumValue).Where(g => g.Count() > 1).OrderByDescending(g => g.Key).ToList();
    ...
    private static string DescribeFace(int value) => value switch
    {
        14 => "Ace",
        13 => "King",
        12 => "Queen",
